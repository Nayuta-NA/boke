<template>
  <div class="articles-view">
    ...
    <template v-else>
      <!-- 动态渲染每个 Tab -->
      <div v-for="(tab, tabIndex) in tabs" :key="tabIndex" v-show="activeTab === tabIndex" class="category-section">
            ...
            <div v-for="(tab, index) in tabs" :key="index" class="category-item">
              ...
              <div class="category-actions">
                <span class="card-type-badge" :class="tab.cardType === 'card1' ? 'card1' : 'card2'">
                  {{ tab.cardType === 'card1' ? '旅游盒子' : '前端/生活盒子' }}
                </span>
                <button 
            ...
            <div class="form-item">
              ...
              <div class="add-category-input">
                ...
                <select v-model="newCategoryCardType" class="form-select card-type-select">
                  ...
                  <option value="card2">前端/生活盒子</option>
                </select>
                <button class="add-category-btn" @click="addCategory" :disabled="!newCategoryName.trim()">
                  <PlusOutlined />
                  添加
                </button>
              </div>
...
const citiesMap: Record<string, string[]> = {
  ...
  '新疆维吾尔自治区': ['乌鲁木齐市', '克拉玛依市', '吐鲁番市', '哈密市', '昌吉回族自治州', '博尔塔拉蒙古自治州', '巴音郭楞蒙古自治州', '阿克苏地区', '喀什地区', '和田地区', '伊犁哈萨克自治州', '塔城地区', '阿勒泰地区'],
  '香港特别行政区': ['香港岛', '九龙', '新界'],
  '澳门特别行政区': ['澳门半岛', '氹仔', '路环']
}
...
const deleteCategory = async (index: number) => {
  ...
  Modal.confirm({
    ...
    onOk: async () => {
      try {
        const response = await fetch(`http://localhost:5000/api/db/articles/categories/${category.id}`, {
          method: 'DELETE',
        })
